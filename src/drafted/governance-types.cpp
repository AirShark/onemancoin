...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "ONEMNetwork") type = ONEMNetwork;
    if(strType == "ONEMNetworkVariable") type = ONEMNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == ONEMNetwork) s = "ONEMNetwork";
    if(type == ONEMNetworkVariable) s = "ONEMNetworkVariable";
