...
const char * const BITCOIN_CONF_FILENAME = "ONEM.conf";
const char * const BITCOIN_PID_FILENAME = "ONEMd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ONEM" is a composite category enabling all ONEM-related debug output
            if(ptrCategory->count(string("ONEM"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ONEMCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ONEMCore
    // Mac: ~/Library/Application Support/ONEMCore
    // Unix: ~/.ONEMcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ONEMCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ONEMCore";
#else
    // Unix
    return pathRet / ".ONEMcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ONEM.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ONEM.conf
